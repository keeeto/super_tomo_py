...
class random_sample(object):
    ...
    def set_pars(self, sz=50, min_shapes=2, max_shapes=15, min_size = 1, max_size=50):
    ...
    def create_image(self):
        ...
        self.im, _ = random_shapes((self.sz, self.sz), min_shapes=self.min_shapes, max_shapes=self.max_shapes, multichannel=False,
                         min_size=self.min_size, max_size=self.max_size, allow_overlap=True)
        self.im = np.where(self.im==255, 1, self.im)
        
        # self.im = self.cirmask(1)
        if np.max(self.im)>0:
